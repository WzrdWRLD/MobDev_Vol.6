...
public class MySQLite extends SQLiteOpenHelper {
    ...
    static final String DATABASE_NAME = "VC"; // Имя базы данных

    static final String TABLE_NAME = "vac_clean"; // Имя таблицы
    static final String ID = "id"; // Поле с ID
    static final String NAME = "name"; // Поле с наименованием книги
    static final String NAME_LC = "name_lc"; // // Поле с наименованием книги в нижнем регистре
    ...
    static final String RATINGP = "ratingP"; // Поле с рейтингом людей
    static final String POWER = "power"; // Поле с мощностью
    static final String LINK = "link"; // Поле с ссылкой

    static final String ASSETS_FILE_NAME = "vacuum_cleaner.txt"; // Имя файла из ресурсов с данными для БД
    ...
    public void addData(SQLiteDatabase db, String name, String description, int price, String ratingP, String power, String link) {
        ...
        try {
            ...
            while ((str = in.readLine()) != null) { // Читаем строку из файла
                ...
                if (!strTrim.equals("")) { // Если строка не пустая, то
                    ...
                    addData(db, name, description, Integer.parseInt(price), ratingP, power, link);
                }
            }
    ...
    public String getData(String filter, Spinner spinner) {
        ...
        if (cursor.moveToFirst()) { // Если есть хоть одна запись, то
            do { // Цикл по всем записям результата запроса
                ...
                int pow = cursor.getColumnIndex(POWER);
